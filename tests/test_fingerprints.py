...
class TestFingerprint:
    ...
    def test_automatic_ifps_creation(self):
        ...
        assert expected_entries == set([e.to_string() for e in pli_obj.entries])
        ...
        assert exists(ifp_out1), f"File '{ifp_out1}' was not found."
        ...
        assert expected_df.equals(curr_df)

        pli_obj.calc_ifp = True
        pli_obj.ifp_length = 4
        pli_obj.generate_fps()
        curr_df = pd.read_csv(ifp_out1)
        assert not expected_df.equals(curr_df)
    ...
    def test_manual_ifps_creation(self):
        ...
        atm_grps_len = [(r.entry.to_string(), len(r.atm_grps_mngr))
                        for r in results]
        assert expected_results["atm_grps_len"] == atm_grps_len

        inter_len = [(r.entry.to_string(), len(r.interactions_mngr))
                     for r in results]
        assert expected_results["inter_len"] == inter_len

        idx = 0
        ifp_as_str1 = self._generate_manual_ifp(results[idx].atm_grps_mngr)
        assert expected_results["ifps"][idx] == ifp_as_str1

        idx = 1
        ifp_as_str2 = self._generate_manual_ifp(results[idx].atm_grps_mngr)
        assert expected_results["ifps"][idx] == ifp_as_str2

        assert ifp_as_str1 != ifp_as_str2
        ...
        ifp_as_str = self._generate_manual_ifp(results[idx].atm_grps_mngr,
                                               ifp_num_levels=3)
        assert expected_results["ifps"][idx] != ifp_as_str
